...
impl AppState {
    ...
    pub async fn pin_context_to_chat_inner(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            commands::context_inputs::compose(&home, &checked, &[recipient], true)
                .map(|_| ())
...
///
/// `macro_rules!`, a nie ciało [`command_handler`], z jednego powodu: ta lista jest deklaracją,
/// a nie algorytmem, i nie ma w niej szwu, wzdłuż którego dałoby się ją przeciąć na dwie
/// funkcje. Handler bierze `Invoke` **przez wartość**, więc pierwsza połowa zjadłaby żądanie,
/// zanim druga zdążyłaby spytać o nazwę — a `there_is_exactly_one_handler_list` zabrania
/// drugiej listy rejestracji w tym pliku wprost. Stojąc w makrze, lista przestaje być ciałem
/// jakiejkolwiek funkcji, więc nie musi udawać podziału, którego tu nie ma, i zostaje co do
/// nazwy i co do kolejności tym samym, czym była.
macro_rules! every_command_the_window_can_call {
    () => {
...
/// Handler, który builder okna dostaje raz — jedyne miejsce, które woła listę wyżej.
///
...
pub fn command_handler() -> impl Fn(Invoke<tauri::Wry>) -> bool + Send + Sync + 'static {
    every_command_the_window_can_call!()
}
