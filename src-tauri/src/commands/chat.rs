...
async fn carry_chat_context(
    ...
    let prepared = tokio::task::spawn_blocking(move || {
        ...
        super::context_inputs::prepare_pins(&home, &selected.sets, &[recipient], true)
    })
    .await
    .map_err(|error| ChatError::CouldNotStart(error.to_string()))?
    .map_err(|refusal| ChatError::CouldNotStart(refusal.message))?;
    let context = prepared.prompt_for("_lead");
    /* 2026-09-08 (scalenie CT-07 z WP-04b) — rozmowa przechodzi przez TEN SAM kompozytor,
     * co krok biegu. Wklejenie bloku wprost przed prompt omijało wspólny przydział, a to jest
     * dokładnie ta wada, którą WP-04b naprawił dla kroków: trzy źródła liczone osobno mogły
     * dołożyć potrójny limit przed instrukcjami vendora. Rdzeń Planu jest tu `None` — rozmowa
     * Leada nie niesie planu. */
    let composed = super::context_inputs::compose("This conversation", "_lead", None, &context, "")
        .map_err(|refusal| ChatError::CouldNotStart(refusal.message))?;
    if !composed.prompt.is_empty() {
        told.spec.prompt = format!("{}\n{}", composed.prompt, told.spec.prompt);
    }
