...
//! procesu, wyjścia i limitu mieszka w tym jednym miejscu (niezmiennik 23). Do 2026-09 (Z-34)
//! stała ona po trzydzieści linii w każdym sterowniku i różniła się między nimi już w chwili,
//! w której powstała druga kopia.
...
use crate::engine::supervisor::{self, AgentCliSearch, DEFAULT_GRACE, GroupProof};
...
/// Rozstrzyga brak celu **przed** startem, bo kod 127 nie odróżnia nieobecnego programu od
...
async fn prove_dead(process: &mut supervisor::Supervised) {
    loop {
        // 2026-09 — sprzątanie nie dziedziczy pięciu sekund programu. Skrócenie tej pętli
...
pub async fn run(binary: &Path, configuration: &DriverConfiguration) -> anyhow::Result<Probe> {
    ...
    /* WPROST, BEZ `env -i` I BEZ ŻADNEJ INNEJ SKORUPY POŚREDNIEJ (2026-09, Z-34). Zawężenie
     * środowiska do samego `PATH` robi dziś [`supervisor::spawn_probe`], więc wrapper nie
     * kupowałby już niczego, a kosztowałby dwie rzeczy naraz: program, którego samego trzeba
     * najpierw znaleźć na `PATH` dziecka, i cudzy kod wyjścia (126/127) wchodzący na miejsce
     * naszego. Argument jest dokładnie jeden, a stdin dostaje EOF natychmiast — bez tego
     * `claude` czeka ~3 s na ostrzeżenie o pustym wejściu [T1 §4.6]. */
    let mut command = Command::new(binary);
    command.arg("--version");

    let mut process = supervisor::spawn_probe(command, &child_path)?;
