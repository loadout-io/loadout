...
use std::fs;
// `MetadataExt` to jedyny sposób zapytać o i-węzeł. Wolno go tu użyć: niezmiennik 3 dotyczy
// kodu wysyłanego, a `checks/quick-boundary.sh` wyłącza pliki testowe po ścieżce.
...
/// układzie względnym, w którym leżą w kopii kanonicznej.
fn assert_installed(dir: &Path, doc: &str) {
    assert_eq!(
