//! AC-2 dla T-23: w pliku są dokładnie dwa rodzaje kroku, a trzeci nie istnieje w schemacie.
//!
...
/// Rodzaje, które schemat zna (T3 §3.1). To jest cała lista i ma taka zostać.
const KNOWN: [&str; 2] = ["agent", "checkpoint"];

/// Rodzaj, którego w schemacie nie ma — i który T-23 zgłasza jako brakujący: kafelek, który sam
/// uruchamia komendę należącą do Loadouta i sam wystawia wynik.
const UNKNOWN: &str = "check";
...
#[test]
fn the_file_holds_exactly_two_kinds_of_step() -> Result<(), Box<dyn Error>> {
    let kinds = kinds(&document()?)?;
    let known: BTreeSet<String> = KNOWN.iter().map(|kind| (*kind).to_owned()).collect();
