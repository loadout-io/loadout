//! T-149 AC-2: Stop wraca dopiero **po** dowodzie, że grupa procesów nie żyje.
//!
...
async fn real_stop_proves_the_term_resistant_group_dead() -> Result<(), Box<dyn Error>> {
    let bench = Bench::new()?;
    ...
    let watching = async {
        ...
        Ok::<GroupId, Box<dyn Error>>(group)
    };
    ...
    assert_durable_cleanup(&report.dir, group, &bench)?;
    Ok(())
}

fn assert_durable_cleanup(
    report_dir: &Path,
    group: GroupId,
    bench: &Bench,
) -> Result<(), Box<dyn Error>> {
    let run: serde_json::Value = serde_json::from_slice(&fs::read(report_dir.join("run.json"))?)?;
    let step = run
        .pointer("/steps/0")
        .ok_or("the cancelled run has no durable step")?;
    assert_eq!(
        step.get("pgid").and_then(serde_json::Value::as_i64),
        Some(i64::from(group.pgid)),
        "run.json did not preserve the actual process group"
    );
    assert_eq!(
        step.get("death_proof").and_then(serde_json::Value::as_bool),
        Some(true),
        "run.json did not preserve the kernel-backed death proof"
    );
    let worktrees = bench.git(&["worktree", "list", "--porcelain"])?;
    assert_eq!(
        worktrees
            .lines()
            ...
            .count(),
        1,
        "a registered step worktree survived Stop:\n{worktrees}"
    );
    Ok(())
}
...
impl Bench {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        bench.git(&["add", "-A"])?;
        bench.git(&["commit", "--quiet", "-m", "stop oracle seed"])?;
        Ok(bench)
    }
