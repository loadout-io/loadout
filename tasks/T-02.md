...
`AGENTS.md` §3 — niezmienniki 1, 7, 8, 11, 19, 24.
`harness/gate.py` — sekcja `contract_problems`: jedno kryterium = jedna ścieżka testu, globalnie
unikalna. Dlatego siedem kryteriów to siedem plików w `src-tauri/tests/`.
...
wewnątrz modułu złamałaby regułę „jedno kryterium, jedna ścieżka pliku" z `harness/gate.py`.
