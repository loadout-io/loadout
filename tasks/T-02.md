...
**Czego potrzebujesz poza własnym kodem.** Moduł `engine` musi być zadeklarowany w
`src-tauri/src/lib.rs` — plik tworzy T-01, a ty masz go w OWNS **wyłącznie** po to, żebyś
dopisał `pub mod engine;`, jeśli tego wiersza jeszcze tam nie ma. Żadnej innej zmiany w tym
pliku. Nie dopisuj sobie ścieżki do `TASK.md` — `checks/quick-scope.sh`
...
- **19 — kod wyjścia to nie dowód.** `cargo test --test it X::` na pliku, w którym wszystkie testy są
...
check: cargo test --test it engine_overlap::
...
check: cargo test --test it engine_concurrency_limit::
...
check: cargo test --test it engine_order::
...
check: cargo test --test it engine_dag_construction::
...
check: cargo test --test it engine_cone_reason::
...
check: cargo test --test it engine_cancel_outcome::
...
check: cargo test --test it engine_step_states::
...
src-tauri/tests/it/engine_overlap.rs
src-tauri/tests/it/engine_concurrency_limit.rs
src-tauri/tests/it/engine_order.rs
src-tauri/tests/it/engine_dag_construction.rs
src-tauri/tests/it/engine_cone_reason.rs
src-tauri/tests/it/engine_cancel_outcome.rs
src-tauri/tests/it/engine_step_states.rs-->
