...
**Deklaracja modułu.** `engine/mod.rs` masz w OWNS **wyłącznie** po to, żebyś dopisał
`pub mod supervisor;`, jeśli tego wiersza jeszcze tam nie ma — sprawdź to jako pierwszą rzecz.
Żadnej innej zmiany w tym pliku: reszta `engine/mod.rs` należy do T-02.
...
check: cargo test --test it supervisor_group_death:: -- --include-ignored
...
check: cargo test --test it supervisor_term_then_kill:: -- --include-ignored
...
check: cargo test --test it supervisor_timeout_kills:: -- --include-ignored
...
check: cargo test --test it supervisor_pipe_eof:: -- --include-ignored
...
check: cargo test --test it supervisor_drop_guard:: -- --include-ignored
...
src-tauri/tests/it/supervisor_group_death.rs
src-tauri/tests/it/supervisor_term_then_kill.rs
src-tauri/tests/it/supervisor_timeout_kills.rs
src-tauri/tests/it/supervisor_pipe_eof.rs
src-tauri/tests/supervisor_env_hygiene.rs
src-tauri/tests/it/supervisor_drop_guard.rs-->
