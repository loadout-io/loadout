...
9. Deterministyczny skan i RuleSync są pierwszą warstwą. Na jawne kliknięcie człowieka Claude
   albo Codex może przeanalizować wyłącznie odkażoną kopię plików setupu i przygotować natywne
   agenty oraz workflow. Wynik jest danymi, nie autorytetem: Rust wiąże go z hashami źródła,
   sprawdza zamknięty schemat, istniejące ścieżki, agentów, komendy i cały graf. Niepokryta
   semantyka nadal zostaje blockerem, a analiza sama niczego nie zapisuje ani nie uruchamia.
