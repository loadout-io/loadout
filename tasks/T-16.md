...
Czego **nie** posiadasz, a będzie kusiło: `src-tauri/src/lib.rs` (deklaracja `pub mod memory;`
należy do T-01 — jeśli jej nie ma, to jest sytuacja z AGENTS.md §7, zapytaj człowieka),
`src-tauri/Cargo.toml` (dopisanie zależności to też AGENTS.md §7), `src-tauri/src/store/**` (T-06).
...
i bramka policzy to jako nieprawdziwe czerwone (`harness/gate.py`, `NOT_A_REAL_RED`).
