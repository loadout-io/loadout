...
Pierwsze zadanie fali redesignu. **Zmienia wylacznie slownik, addytywnie** — ani jednej
powierzchni, ani jednego komponentu. Po nim aplikacja ma nowe barwy i nowe kroje przy STARYCH
promieniach, i to jest stan przejsciowy z premedytacja: promienie migruja powierzchnia po
powierzchni w T-46/T-47/T-48, a aliasy gina w T-50 (niezmiennik 25 — migracje sa addytywne).
...
**Wartosci wchodza do repo jako wersjonowana migawka `docs/design/house-tokens.json`, nie jako
...
- `docs/mockup/index.html` — **wylacznie blok `:root`** oraz zdjecie `text-transform:uppercase`
  z dwoch regul: `.fld label` i `.card .role`. Reszta makiety nalezy do T-46/T-47/T-48.
- `docs/design/house-tokens.json` — **nowy**, migawka wartosci domu.
...
w `src-tauri/**`, ani `checks/**`, ani `docs/DECISIONS-LOCKED.md`. Promienie zostaja na 2 px:
`--radius-sq` i `--radius-dot` **zyja** i wskazuja na nowe pasmo.
...
- **21 — nie pisz artefaktu, ktorego zaden skrypt nie czyta.** `house-tokens.json` jest czytany
...
Test czyta `docs/design/house-tokens.json` i `src/styles/theme.css` w tym samym biegu i porownuje
