> **ZAMKNIETE 2026-08-20 — piec z szesciu kryteriow wyladowalo, a szoste zostalo ZASTAPIONE.
> Tego zadania sie NIE URUCHAMIA.**
>
> Zmierzone: z szesciu sciezek testow tego kontraktu brakuje na dysku jednej —
> `src/styles/radius-aliases-still-resolve.test.ts` (AC-4, „aliasy promieni zyja i rozwijaja sie
> do nowego pasma"). Pozostale piec stoi w trunku i przechodzi, wiec bieg odbilby sie od warstwy
> `before` z powodem „kryterium juz przechodzi — nie certyfikuje niczego".
>
> **AC-4 nie jest niespelnione, jest PRZEDAWNIONE — i nie wolno go przywracac.** Lancuch decyzji:
> T-45 wprowadzil pasmo `--radius-sm/md/lg/pill` ADDYTYWNIE, zostawiajac `--radius-sq`
> i `--radius-dot` jako przekierowania, zeby zadna powierzchnia nie zostala bez ani jednej reguly
> CSS w trakcie migracji. T-46, T-47 i T-48 przeniosly powloke, ekran Run i cztery sekcje listowe.
> **T-50 AC-1** (`src/styles/stand-in-names-are-gone.test.ts`, wyladowane) zamknal migracje
> i asertuje teraz, ze tych dwoch nazw NIE MA — z uzasadnieniem, ktore jest tam napisane wprost:
> „nazwa zastepcza, ktora przezyje fale, nie jest juz migracja — jest druga nazwa tej samej rzeczy
> (niezmiennik 13)".
>
> Odtworzenie aliasow spelniloby wiec AC-4 i **zapalilo kryterium T-50**. Dwa kryteria nie moga
> byc prawdziwe naraz, a rozstrzyga je kolejnosc: pozniejsza, wyladowana decyzja wygrywa.
> Zmierzone dzis: `--radius-sm/md/lg/pill` istnieja w `theme.css` (wiersze 256-259), a `--radius-sq`
> i `--radius-dot` nie wystepuja w zadnym pliku `src/` poza komentarzami opisujacymi te historie.
>
> Zostaje wiec jako zapis, nie jako robota. Nic z tego kontraktu nie jest do zrobienia.
