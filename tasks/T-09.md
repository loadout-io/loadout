...
linie 438–477 — sesja: `What Forge was given` / `What Forge produced` / `Its session`, w tej
kolejności),
...
Dwie rzeczy, których to zadanie **nie** naprawia, choć na nie natrafi:
`src/styles/theme.css` nie definiuje `--color-id-1…--color-id-5`, przez co `checks/quick-tokens.sh`
jest dziś czerwony niezależnie od tej gałęzi, a plik nie należy do żadnego zadania. Zgłoś to
(AGENTS.md §7) i nie edytuj go. Twoje moduły operują na **nazwach** tokenów (`'--color-id-3'`),
komponenty piszą `var(--color-id-3)` — żadnego hexa w kodzie komponentu (DESIGN.md §9).
...
`Its session` (makieta, linie 449–467). Wiersze `given` mają rodzaje wyłącznie ze zbioru
