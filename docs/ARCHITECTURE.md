...
| 4 | Co obiecujemy w kwestii izolacji? | **Mówimy prawdę, jednym zdaniem, w UI: „każdy krok dostaje własną kopię twoich plików".** To izolacja współbieżności, nie sandbox bezpieczeństwa. Sandboxa nie ma w v1. | poprzedni prototyp to zapisał w `docs/security.md`, ale nie powiedział tego użytkownikowi. Obietnica bezpieczeństwa, której nie dowozimy, jest gorsza niż jej brak. | Niski — sandbox dokłada się później jako polityka na agencie. |
