...
| H16 | L2: odrzucona notatka wraca — `record()` nie zagląda do `discarded/` | **T-139** (T-104/T-128/T-136/T-137/T-138 zamknięte; T-139 mierzy oba korzenie) |
| H17 | `Block::dropped` bez konsumenta; etykieta ekranu kłamie o zasięgu; `from` przeciążone | **T-129 → T-130** po adresie T-139 |
| H18 | L1: pamięć wyłącznie globalna — `this-project` przecieka między repo | **T-139** (D-2 = TAK; T-138 zamknięte po 18/19 i luce tombstone'a) |
...
| T-139 | Lądowalny oracle dwóch korzeni z Clippy od pierwszego commita | T-127 (pełny następca T-138) | tak | 3 |
...
**T-139 — lądowalny oracle dwóch korzeni.** Świeży następca T-138 ma trzy nowe targety.
Zachowuje pełne pokrycie realnej refleksji, snapshotu, Move i UI, ale każda funkcja testowa ma
od początku najwyżej 90 linii. Osobna scena sadzi exact tombstone tylko w bibliotece i woła
`record_project_candidate_from_run`, wymagając odmowy zapisu projektu; prefix-extra jest
kontrolą negatywną. Dopiero po uczciwym `before` wolno użyć siedmiu commitów produkcyjnych
T-138; jego kontraktu, speców, pustego commita i naprawy testu nie przenosić.
...
- **T-114, T-100, T-101, T-115, T-121, T-124, T-126 i T-127 wylądowały; T-102, T-103, T-104, T-109, T-116, T-117, T-118, T-119, T-120, T-122, T-123, T-125, T-128, T-136, T-137 i T-138 są zamknięte, T-139 jest następne.**
  Nie wznawiać zamkniętych gałęzi ani nie przenosić ich testów, implementacji lub commitów,
  z jednym jawnym wyjątkiem: po certyfikowanym `before` T-139 może zastosować wyłącznie
  `705f433`, `7d9bbc9`, `124cc46`, `6642567`, `5ceea68`, `d439a25` i `3dba18d` z T-138, bo nowe oracle powstają od zera w świeżym biegu.
  ...
  domknęło H29 po niewykonalnym T-109, a T-139 przejmuje H16/H18 po zamkniętych T-104/T-128/T-136/T-137/T-138.
- **Łańcuch `run.rs`** (dzielony OWNS, więc szeregowo):
  `T-114 → T-100 → T-101 → T-102 (zamknięte) → T-115 → T-103…T-120 (zamknięte) → T-122 (zamknięte) → T-124 → T-123 (zamknięte) → T-125 (zamknięte) → T-126 → T-127 → T-128 (zamknięte) → T-136 (zamknięte) → T-137 (zamknięte) → T-138 (zamknięte) → T-139 → T-129 → T-130 → świeże zadania Stop/startup`.
...
- **T-127 po T-126 wylądowało.** T-128 zamknięto na dwóch starych testach poza OWNS, T-136
  po czerwonej bramce i lukach oracle, T-137 po 17/19, a T-138 po 18/19 i luce tombstone'a; T-139 przejmuje pełny zakres i musi zamrozić adres
