...
| H16 | L2: odrzucona notatka wraca — `record()` nie zagląda do `discarded/` | **T-138** (T-104/T-128/T-136/T-137 zamknięte; T-138 mierzy exact `<id>__`) |
| H17 | `Block::dropped` bez konsumenta; etykieta ekranu kłamie o zasięgu; `from` przeciążone | **T-129 → T-130** po adresie T-138 |
| H18 | L1: pamięć wyłącznie globalna — `this-project` przecieka między repo | **T-138** (D-2 = TAK; T-137 zamknięte po 17/19 i trzech lukach oracle) |
...
| T-138 | Dwa korzenie z działającą refleksją, śladem Move po sukcesie i legacy we właściwej strefie | T-127 (pełny następca T-137) | tak | 3 |
...
bramce i lukach oracle; T-137 zamknięto 17/19, a zakres przejmują świeże T-138 → T-129 → T-130. Niczego nie przenosi
...
**T-138 — działające wyrocznie refleksji, Move i widocznej strefy.** Świeży następca T-137
ma trzy nowe standalone targety. Refleksja przechodzi pełny produkcyjny łańcuch wrapperów,
receipt i właściwy fizyczny korzeń; snapshot używa niekanonicznego front mattera i porównania
bajtów po normalizacji wyłącznie stempla. RecordingMoveIo dopisuje zdarzenie dopiero po
udanym delegowaniu, a odmowa nie może udawać wykonanej operacji. Browserowy oracle lokalizuje
legacy wewnątrz `earlier-project`, dowodzi jego nieobecności w `suggested` i realnie klika
Move, Use, Stop i Discard. Dopiero po uczciwym `before` wolno użyć pięciu commitów
implementacyjnych T-137; jego kontraktu i speców nie przenosić.
...
- **T-114, T-100, T-101, T-115, T-121, T-124, T-126 i T-127 wylądowały; T-102, T-103, T-104, T-109, T-116, T-117, T-118, T-119, T-120, T-122, T-123, T-125, T-128, T-136 i T-137 są zamknięte, T-138 jest następne.**
  Nie wznawiać zamkniętych gałęzi ani nie przenosić ich testów, implementacji lub commitów,
  z jednym jawnym wyjątkiem: po certyfikowanym `before` T-138 może zastosować wyłącznie
  `452bc47`, `b649929`, `db56f0e`, `e772354` i `8345b02` z T-137, bo nowe oracle powstają od zera w świeżym biegu.
  ...
  domknęło H29 po niewykonalnym T-109, a T-138 przejmuje H16/H18 po zamkniętych T-104/T-128/T-136/T-137.
- **Łańcuch `run.rs`** (dzielony OWNS, więc szeregowo):
  `T-114 → T-100 → T-101 → T-102 (zamknięte) → T-115 → T-103…T-120 (zamknięte) → T-122 (zamknięte) → T-124 → T-123 (zamknięte) → T-125 (zamknięte) → T-126 → T-127 → T-128 (zamknięte) → T-136 (zamknięte) → T-137 (zamknięte) → T-138 → T-129 → T-130 → świeże zadania Stop/startup`.
...
- **T-127 po T-126 wylądowało.** T-128 zamknięto na dwóch starych testach poza OWNS, T-136
  po czerwonej bramce i lukach oracle, a T-137 po 17/19 i martwych granicach testów; T-138 przejmuje pełny zakres i musi zamrozić adres
